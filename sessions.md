...
| # | Status | Note | Commit / PR |
|---|---|---|---|
| 1.0a | ⏳ | **NEW 2026-05-07** — fleet Go 1.25 → 1.26 bump. Surfaced when 2.7.1 needed `k8s.io/client-go` v0.32 (requires Go 1.26). Touches: `leartech-dockerfiles/leartech-go-runtime/Dockerfile` FROM line + 4 catalog task images (`go/release.yaml`, `go/pullrequest.yaml`, `govulncheck/pullrequest.yaml`, `go-test/pullrequest.yaml`) + Renovate-driven go.mod bumps across consumer services. Unblocks 2.7.1 deploy. ~2-3h with smoke testing across services. | — |
| 1.0b | ⏳ | **NEW 2026-05-07** — AZ release pipeline silently skips image push (`auth-service` ACR latest is 0.1.35 from 2026-05-05, despite GCP releasing 0.1.36-0.1.38). Investigate AZ kaniko + build-pack step. ~2h. | — |
| 1.0c | ⏳ | **NEW 2026-05-07** — CNPG 1.29.0 doesn't start the Database controller (only cluster/backup/scheduled-backup/plugin/pooler controllers run). Database CRDs accept-validate fine but nothing reconciles. Manual `CREATE DATABASE` workaround used today during auth-service cutover. Either upgrade CNPG when fixed OR add a Helm pre-install hook that creates databases via SQL (mqube-Maestro pattern). | — |
| 1.0d | ⏳ | **NEW 2026-05-07** — auto-trigger Renovate after shared-library releases. Today's Go 1.26 fleet bump exposed: shipping a new `leartech-go-runtime` (or `leartech-go-common`) version means consumers wait until next Monday 7am for Renovate to open bump PRs. Manual workaround: `kubectl create job --from=cronjob/renovate "renovate-manual-$(date +%s)"`. Fix: append a final step to leartech-dockerfiles + leartech-go-common release.yaml that creates that Job. Renovate then opens grouped + auto-merging PRs (already configured: `groupName: leartech-go-runtime`/`leartech internal libs`, `automerge: true`). | — |
...
| # | Status | Note | Commit / PR |
|---|---|---|---|
| 2.7.1 | 🚧 | **partial 2026-05-07 — code complete, deploy BLOCKED on fleet Go bump**. Repo bootstrapped, Arrival CRD, K8s ReplicaSet informer, RBAC, OTLP — all on `main`. Source-config registered both clusters (PRs #254/#134 merged). Local `make pre-push` green. **Blocker**: `k8s.io/{api,apimachinery,client-go} >= v0.32` requires Go ≥1.26.0; entire `leartech-pipeline-catalog` is hardcoded to `golang:1.25-*` images (`go/release.yaml`, `go/pullrequest.yaml`, `govulncheck/pullrequest.yaml`, `go-test/pullrequest.yaml`). First release pipeline run failed with `go: go.mod requires go >= 1.26.0`. Tried pinning to v0.30/v0.32 — hit the kube-openapi vs yaml.v3 vs structured-merge-diff dep-tangle (indirect deps no longer have a coherent intersection at Go-1.25-compatible k8s versions). Phase 1 hardening: bump `leartech-go-runtime:0.18.0` (Dockerfile FROM `golang:1.25-alpine`) → 0.19.0 (1.26-alpine), bump catalog tasks' golang image refs, Renovate-bump consumer go.mod files. Resume 2.7.1 deploy + 2.7.2 dispatch after fleet bump lands. | repo `mikelear/leartech-arrivals-observer@a4789ed`; gsm#254; akv#134 |
| 2.7.2 | ⏳ | not started | — |
| 2.7.3 | ⏳ | not started | — |
| 2.7.4 | ⏳ | not started | — |
| 2.7.5 | ⏳ | not started | — |
